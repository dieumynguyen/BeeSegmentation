...
'''
Given images and corresponding annotations text files, create cropped images and segmentation masks, with bees drawn as ellipses. 
'''
...
def ellipse_around_point(image, xc, yc, angle, r1, r2, value):
    ...
    'Draw ellipse around an xy point in image. Return drawn image.'
...
def make_crop_mask(cropped_image, df, min_x=None, min_y=None):
...
def setup_args():
    ...
    parser.add_argument('-r', '--data_root', dest='data_root', type=str, default='/home/dieumynguyen/BeeSegmentation/data/',
                        help='Set root path to data [default: /home/dieumynguyen/BeeSegmentation/data/]')
    parser.add_argument('-o', '--overwrite', dest='overwrite', type=int, default=0,
                        help='Overwrite files [default: 0]')
    parser.add_argument('-d', '--OIST_data', dest='OIST_data', type=int, default=1,
                        help='OIST data (or Golnars data)? [default: 1]')
...
def main(args):
    ...
    frames_root = os.path.join(args.data_root, 'source', 'frames')
    ...
    for i, (frame_path, txt_path) in enumerate(zip(frame_paths, txt_paths)):
        ...
        frame_savepath = os.path.join(args.data_root, 'processed', 'images', f'{frame_name}.npy')
        mask_savepath = os.path.join(args.data_root, 'processed', 'masks', f'{frame_name}.npy')
